...
DEVICE static inline Vector3D estimate_direct_lighting_importance(Ray r, const CudaIntersection isect, int idx) {
  ...
  while (l++ < ns_area_light) {
    ...
    if (pdfL > 0 && cosNL > 0) {
      ...
      if (!has_intersect(primitives, vertices, nodes, &shadow)) {
...
DEVICE static inline Vector3D at_least_one_bounce_radiance(Ray r, const CudaIntersection isect_init, int idx) {
...
DEVICE static inline void raytrace_pixel_temporal_sample(uint16_t x, uint16_t y, bool restir) {
  ...
  Vector2D sample;
  sample.x = origin.x + next_float(&rand_states[idx]);
  sample.y = origin.y + next_float(&rand_states[idx]);
  ...
  if (intersect(&r, primitives, vertices, normals, texcoords, tangents, nodes, &isect)) {
    ...
    L = at_least_one_bounce_radiance(r, isect, idx);
    L = accumulate ? vector3d_add(L, get_emission(bsdfs, textures, isect)) : L;
  }
...
DEVICE static inline void spatial_resampling(uint16_t x, uint16_t y) {
  const uint16_t neighbouring_pixel_radius = floor(0.05 * min(w, h));
}
